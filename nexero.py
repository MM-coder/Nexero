...
async def mute(ctx, member: discord.Member, time: int, *, reason):
    ...
    embed = discord.Embed(title="Mute Issued", description="{} You have been Muted for **{}** Seconds. Reason: {}".format(member.mention, time, reason), color=0x08202D)
...
async def ban(ctx, user: discord.Member, *, reason: str):
    if "n.staff" in [role.name for role in ctx.message.author.roles]:
        await bot.ban(user)
        #case = get_bans(ctx.message.server.id)
...
async def rulessetup(ctx):
    if ctx.message.author.id == '279714095480176642':
        ...
        lowsev.add_field(name="False/ Spam pings", value="Like ghost pinging, pinging staff for your meme, etc", inline=False)
